    ...
    <div hidden={props.hidden}>
      <Container className='categoryGroup' maxWidth="md">
        <div className='question' >
          ...
          {state && <h2>{questions[currentQuestion].question}</h2>}
          {state && <div className='answers'>{random.map(value => ({ value, sort: Math.random() })).sort((a, b) => a.sort - b.sort).map(({ value }) => <h3 key={value} value={value} onClick={() => newQuestion({ value })}>{value} </h3>)}</div>}<BiCheck hidden={false} ></BiCheck>
